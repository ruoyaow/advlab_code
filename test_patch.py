...
def patch_apply_custom(imgdir, patchfile, target_class, savedir = "testing"):
    ...
    if not os.path.exists(os.path.join(savedir, 'clean/', 'yolo-labels/')):
        os.makedirs(os.path.join(savedir, 'clean/', 'yolo-labels/'))
    if not os.path.exists(os.path.join(savedir, 'proper_patched/', 'yolo-labels/')):
        os.makedirs(os.path.join(savedir, 'proper_patched/', 'yolo-labels/'))
    if not os.path.exists(os.path.join(savedir, 'random_patched/', 'yolo-labels/')):
        os.makedirs(os.path.join(savedir, 'random_patched/', 'yolo-labels/'))
    ...
    for imgfile in os.listdir(imgdir):
        ...
        if imgfile.endswith('.jpg') or imgfile.endswith('.png'):
            ...
            for box in boxes:
                cls_id = box[6]
                if(cls_id == target_class):   #if person
                    x_center = box[0]
                    y_center = box[1]
                    width = box[2]
                    height = box[3]
            ...
            #'''
            if adv_batch_t[0].squeeze(0).cpu().ndimension() == 4:
                adv_batch_t_pil = transforms.ToPILImage('RGB')(adv_batch_t[0][0].squeeze(0).cpu())
            else:
                adv_batch_t_pil = transforms.ToPILImage('RGB')(adv_batch_t[0].squeeze(0).cpu())
            #'''
...
if __name__ == '__main__':
    patch_apply_custom("testperson", "saved_patches/patch11.jpg", 0)