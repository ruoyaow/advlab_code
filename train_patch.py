...
class PatchTrainer(object):
    def __init__(self, mode, target_class):
        ...
        self.prob_extractor = MaxProbExtractor(target_class, 80, self.config).cuda()
    ...
    def train(self, starter_img = None, patch_save_name = '', n_epochs = 10000):
        """
        ...
        if starter_img is None:
            adv_patch_cpu = self.generate_patch("gray")
        else:
            adv_patch_cpu = self.read_image(starter_img)
        ...
        for epoch in range(n_epochs):
            ...
            if True:
                ...
                #plt.imshow(im)
                #plt.show()
                im.save("saved_patches/patchnew_"+patch_save_name+".jpg")
                del adv_batch_t, output, max_prob, det_loss, p_img_batch, nps_loss, tv_loss, loss
                torch.cuda.empty_cache()
            et0 = time.time()
